    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1 font-medium opacity-80">
            Oversee and modify all promotional campaigns and discounts.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col relative overflow-hidden">
          ...
          <div className="mt-auto">
            <p className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-1.5">
              Active Coupons
            </p>
      ...
      <div className="space-y-4 mb-10">
        {isLoading ? (
          [...Array(3)].map((_, i) => (
            <div
              key={i}
              className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm animate-pulse h-24"></div>
          ))
        ) : coupons.length > 0 ? (
          coupons.map((coupon) => (
            <div
              key={coupon._id}
              className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm grid grid-cols-1 md:grid-cols-12 gap-5 items-center">
              {/* Left: Code & Info */}
              <div className="flex items-center gap-4 md:col-span-5">
                ...
                <div className="min-w-0">
                  <h4 className="text-lg font-bold text-gray-900 tracking-tight leading-none mb-1">
                    {coupon.code}
                  </h4>
              ...
              <div className="md:col-span-4 flex items-center gap-8 pl-0 md:pl-4">
                ...
                <div className="flex flex-col">
                  ...
                  <span
                    className={`text-[11px] font-bold ${coupon.validityInfo === "Expired" ? "text-red-500" : "text-gray-500"}`}>
                    {coupon.validityInfo}
                  </span>
                </div>
              </div>
              ...
              <div className="md:col-span-3 flex items-center justify-between md:justify-end gap-3 border-t md:border-0 pt-4 md:pt-0">
                <span
                  className={`px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest border shadow-sm ${coupon.status === "Active" ? "bg-green-50 text-green-600 border-green-100" : "bg-red-50 text-red-600 border-red-100"}`}>
                  {coupon.status}
                </span>
          ...
          <div className="bg-white rounded-[32px] border border-gray-100 p-20 text-center shadow-inner bg-gray-50/20">
            ...
            <p className="text-gray-500 text-sm font-medium">
              Create your first promotional campaign to boost your platform
              growth.
            </p>
          </div>
        )}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-4 border-t border-gray-100">
        <div className="text-xs md:text-sm font-medium text-gray-500 order-2 sm:order-1">
          ...
          <span className="font-bold text-gray-900">
            {pagination.totalCoupons === 0 ? 0 : (page - 1) * PAGE_LIMIT + 1}-
            {Math.min(page * PAGE_LIMIT, pagination.totalCoupons)}
          </span>{" "}
          of{" "}
          <span className="font-bold text-gray-900">
