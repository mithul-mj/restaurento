...
export const getAllCoupons = async (req, res, next) => {
    try {
        ...
        const formattedCoupons = coupons.map(coupon => {
            const isExpired = coupon.expiryDate && new Date(coupon.expiryDate) < dateNow;
            let validityInfo = "";
            ...
            return {
                ...
                discountDesc: coupon.maxDiscountCap ? `Up to ₹${coupon.maxDiscountCap}` : "No upper limit",
                validityDate,
                validityInfo,
...
export const updateCoupon = async (req, res, next) => {
    try {
        ...
        if (code && code.toUpperCase().trim() !== coupon.code) {
            ...
            if (existingCoupon) {
                throw new ApiError(STATUS_CODES.CONFLICT, "New coupon code already exists");
            }
        ...
        const [usageCount, usageStats, dailyHistory] = await Promise.all([
            ...
            Booking.aggregate([
                ...
                },
                {
                    $group: {
                        _id: null,
                        totalRedeemedValue: { $sum: "$appliedCoupon.discountAmountApplied" },
